...
export default class RTCConnection extends EventEmitter {
  constructor(options: RTCCOptions) {
    ...
    this.comm = new CommClient(
      this.id,
      new CommClient.SignalingSocket(
        new WS(
          options.gateway + `?id=${this.id}&transport=${this.options.transport}`
        )
      ),
      { target: "CLIENT" }
    );
  ...
  public createAudio() {
    return new Promise<Audio>((resolve, reject) => {
      this.createAudioSync(function (audio, err) {
        if (err) reject(err);
        else resolve(audio);
      });
    });
  ...
  public createAudioSync(
    callback?: (audio: Audio, err?: Error | null) => void
  ) {
    navigator.mediaDevices
      ...
      .then((_stream) => {
        ...
        let {
          audio: { _, context, info },
        } = this;
        _.analyser = context.createAnalyser();
        _.analyser.channelCount = 1;
        _.analyser.fftSize = 64;
        _.analyser.maxDecibels = 0;
        _.analyser.smoothingTimeConstant = 0.5;
        _.func = () => {
          const byteFrequencies = new Uint8Array(1),
            { maxDecibels, minDecibels } = _.analyser;
          _.analyser.getByteFrequencyData(byteFrequencies);

          info.dB = Number(
            (
              minDecibels +
              ((maxDecibels - minDecibels) * byteFrequencies[0]) / 255
            ).toFixed()
          );
          info.last_dB = Number(
            (
              info.last_dBs.reduce((prev, current) => prev + current, 0) /
              info.last_dBs.length
            ).toFixed()
          );
          if (info.last_dBs.length === 5) info.last_dBs.shift();
          info.last_dBs.push(info.dB);

          info.dB_difference = Math.abs(info.last_dB - info.dB);
          info.thresholds["dynamic"] =
            info.thresholds["static"] - info.dB_difference;
  ...
  public createCam() {
    return new Promise<Cam>((resolve, reject) => {
      this.createAudioSync(function (cam, err) {
        if (err) reject(err);
        else resolve(cam);
      });
    });
  ...
  public createDisplay() {
    return new Promise<Display>((resolve, reject) => {
      this.createAudioSync(function (display, err) {
        if (err) reject(err);
  ...
  public createDisplaySync(
    callback?: (display: Display, err?: Error | null) => void
  ) {
    navigator.mediaDevices
  ...
  public emit<K extends keyof RTCCEvents>(
    eventName: K,
    ...args: RTCCEvents[K]
  ): boolean {
  ...
  public on<K extends keyof RTCCEvents>(
    eventName: K,
    listener: (...args: RTCCEvents[K]) => void
  ): this {
