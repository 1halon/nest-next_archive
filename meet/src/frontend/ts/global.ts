...
export class IndexedDB extends EventEmitter {
  constructor(name: string, version?: number) {
    ...
    Object.defineProperties(this, {
      name: { value: name, writable: !1 },
      version: { value: version, writable: !1 },
    });
    ...
    request.addEventListener("error", () => {
      ...
      this.state = IndexedDBState["FAILED"];
    });
    ...
    request.addEventListener("success", () => {
      ...
      this.state = IndexedDBState["SUCCESS"];
      this.emit("ready");
    });
  }
  ...
  delete() {
    return new Promise<void>((resolve, reject) =>
      this.deleteSync((result) =>
        result === true ? resolve() : reject(result)
      )
    );
  }
  ...
  on<K extends keyof IndexedDBEvents>(
    event: K,
    listener: (data: IndexedDBEvents[K]) => void
  ): this {
...
export class IndexedDBManager extends EventEmitter {
  ...
  delete(name: string) {
    this.indexedDBs[name]
      ?.delete()
      .then(() => delete this.indexedDBs[name])
      .catch((e) => void e);
  }

  on<K extends keyof IndexedDBManagerEvents>(
    event: K,
    listener: (data: IndexedDBManagerEvents[K]) => void
  ): this {
    return EventEmitter.on.apply(this, arguments);
  }
}
  ...
  if (!Array.isArray(object) && typeof object === "object")
    for (const key of Object.keys(object))
      for (const element of [...document.getElementsByClassName(key)]) {
        ...
        element.classList.add(object[key]);
      }
}
