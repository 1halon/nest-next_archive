...
export class TemplateService {
  public readonly templates_dir = join(
    process.cwd(),
    'client',
    'private',
    'templates'
  ...
  getAllTemplates() {
    ...
    for (const category of readdirSync(this.templates_dir)) {
      if (extname(category) !== '') return;
      templates[category] = {};
      for (const [template, template_function] of this.getAllCategoryTemplates(
        category
      ))
  ...
  getAllCategoryTemplates(category: string) {
    ...
    for (const template of readdirSync(join(this.templates_dir, category)))
      templates[template.split('.')[0]] = this.compileTemplate(
        `${category}/${template}`
      );
    return templates;
  }
