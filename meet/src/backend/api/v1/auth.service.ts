...
export class AuthServiceV1 {
  ...
  public readonly confirmation_requests = {} as Record<
    string,
    ConfirmationRequest
  >;
  ...
  private static readonly RequestHandlers = {
    ...
    async REGISTER(request: ConfirmationRequest) {
      return function (req: Request, res: Response) {
        ...
        new this.userService.userModel(userModel)
          .save()
          .then(function () {
            res.send(pass);
          })
  ...
  createRequest(
    ...
    const ex_requests = Object.values(this.confirmation_requests).filter(
      (request) => request.username === username
    );
    if (
      type === 'REGISTER' &&
      ex_requests.findIndex((request) => request.type === 'REGISTER') !== -1
    )
      throw new Error('CONFIRMATION_IN_PROGRESS');
    else if (
      type === 'LOGIN' &&
      ex_requests.filter((request) => request.identifier === identifier)
        .length > 5
    )
    ...
    request.self_destruct = setTimeout(
      () => request.resolve(),
      AuthServiceV1.RequestTimeout
    );
